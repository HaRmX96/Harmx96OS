...
; Disk input can start anywhere (OS ) or Buf0 (DOS)
HEADING 'TIB'			; ( -- tib )
  TIB:		dw create
  tib1:		dw stack0
...
; OSwill read whole file; make sure it will fit!
HEADING 'FREAD'
...
; OSwill not overwrite an existing file
HEADING 'FWRITE'
...
%if OS 
; OSget file size to ensure it will fit in available memory
HEADING 'F_SIZE'
...
; OSread entire file - it must fit in memory without hurting OSor Forth
...
; Note - no address can be less than 0100h (.com) or 8000h (OS )
...
; LFA since > 0100h (DOS) or 0x8000 (OS ), can be used as "found" flag
...
; OSis not a good fit for nesting.
HEADING "INCLUDE"		; ( -- )
		dw colon
...
HEADING 'D_VER'			; DOS or OSAPI version - display purposes
  D_VER:        dw create
  dver1:        dw      0
...
start_forth:
	...
	db      sf01 - $ - 1
%if OS 
	db      'Copyright (C) 2022 HaRmX96 Developers -- see doc/LICENSE.TXT'
%else
	db      'Copyright 2022, all rights reserved.'
%endif
  sf01:
	dw      CR, dotq
	db      sf02 - $ - 1
